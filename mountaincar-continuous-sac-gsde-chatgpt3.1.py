...
class SACConfig:
    ...
    target_entropy: Optional[float] = -1.0 # None  # if None uses -action_dim
    use_auto_alpha: bool = True
    init_alpha: float = 0.2
    # gSDE
    use_gsde: bool = True
    sde_sample_freq: int = 32  # -1: E fixed during episode; helps stability
    use_sde_at_warmup: bool = False
    feature_dim: int = 64
    log_std_init: float = -3.67 # -1.0
    # obs normalization
...
class SACAgent:
    ...
    def evaluation(self):
        ...
        for _ in range(n_eval):
            ...
            while terminated == False and truncated == False:
                ...
                ep_step += 1
                ep_ret += rew
        return ep_ret/n_eval, ep_step/n_eval
    ...
    def train(self):
        ...
        for step in range(1, self.cfg.total_steps + 1):
            ...
            if step < self.cfg.start_steps and not (self.cfg.use_gsde and self.cfg.use_sde_at_warmup):
            ...
            if done or (ep_len >= env.spec.max_episode_steps):
                ...
                if self.cfg.use_gsde:
                    sigma_val = float(self.actor.noise.sigma.detach().cpu().numpy())
                if self.log_alpha is not None:
                    alpha_val = float(torch.exp(self.log_alpha).detach().cpu().numpy())
                obs, _ = env.reset()
                obs = obs
                stack_ep_ret.append(ep_ret)
                stack_ep_len.append(ep_len)
                ep_ret, ep_len = 0.0, 0
            ...
            if step % 1000 == 0:
                avg_ep_ret = np.mean(stack_ep_ret)
                avg_ep_len = np.mean(stack_ep_len)
                stack_ep_ret = []
                stack_ep_len = []
                ev_ret, ev_len = self.evaluation()
                #ev_ret, ev_len = 0.0 , 0.0
                print(f"Step {step}, Episode return {avg_ep_ret:+5.1f}, length {avg_ep_len:5.1f}, sigma {sigma_val:.4f}, alpha {alpha_val:.3e}, ev ret {ev_ret:+5.1f}, ev len {ev_len:5.1f},")
                history_ep_ret.append(avg_ep_ret)
                history_ep_len.append(avg_ep_len)
                history_ev_ret.append(ev_ret)
                history_ev_len.append(ev_len)
        return history_ep_ret, history_ep_len, history_ev_ret, history_ev_len
...
if __name__ == "__main__":
    ...
    history_ep_ret, history_ep_len, history_ev_ret, history_ev_len = agent.train()
    plt.figure()
    plt.plot(history_ep_ret, label='Episode Reward')
    plt.plot(history_ev_ret, label='Evaluation Reward')
    plt.plot(history_ep_len, label='Episode Length')
    plt.plot(history_ev_len, label='Evaluation Length')
    plt.legend()
    plt.show()
    ...
    for _ in range(n_eval):
        ...
        while terminated == False and truncated == False:
            ...
            ep_step += 1
            ep_ret += rew
        print(f"return {ep_ret}, step {ep_step}")
